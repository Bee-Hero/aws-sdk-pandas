...
from beehero_awswrangler import _config, exceptions
from beehero_awswrangler.__metadata__ import __version__
from beehero_awswrangler._arrow import _table_to_df
from beehero_awswrangler._config import _insert_str, apply_configs
from beehero_awswrangler._distributed import EngineEnum, engine
...
# for example, pip install beehero_awswrangler[oracle] installs oracledb
INSTALL_MAPPING = {
        ...
        def inner(*args: Any, **kwargs: Any) -> Any:
            if not module:
                ...
                raise ModuleNotFoundError(
                    f"Missing optional dependency '{name}'. Use pip install beehero_awswrangler[{install_name}] to install it."
                )
...
def default_botocore_config() -> botocore.config.Config:
    ...
    return Config(
        ...
        user_agent_extra=f"beehero_awswrangler/{__version__}",
    )
...
def parse_path(path: str) -> tuple[str, str]:
    ...
    Examples
    --------
    >>> from beehero_awswrangler._utils import parse_path
...
def ensure_cpu_count(use_threads: bool | int = True) -> int:
    ...
    Examples
    --------
    >>> from beehero_awswrangler._utils import ensure_cpu_count
...
def ensure_worker_or_thread_count(use_threads: bool | int = True) -> int:
    ...
    Examples
    --------
    >>> from beehero_awswrangler._utils import ensure_worker_or_thread_count
...
def chunkify(
    ...
    Examples
    --------
    >>> from beehero_awswrangler._utils import chunkify
